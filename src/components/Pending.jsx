...
import "./Dash.css";
export default function Pending({ data, type }) {
  const pendingReviews = data.filter((review) => { return review.Review_status === "pending" })
  ...
  return (
    <>
      ...
      {pendingReviews.map((review, key) => {
        return (
          <div key={key} className="card  text-center pending-bg-set my-3 bg-light">
            ...
            <div className="d-lg-flex flex-column  mt-5 text-dark my-3 ">
              ...
              {(type === "company" ? (
                ...
                <button onClick={(e)=>{submitStatus(e)}} data-key={review.id} value={"Accepted"} className='btn btn-success my-2 mx-2 text-dark'>
                  Accept
                </button>
                <button onClick={(e)=>{submitStatus(e)}} data-key={review.id} value={"Rejected"} className='btn btn-danger my-2 mx-2'>
                  Reject
                </button>
